...
def patchify(
    ...
    1. Optionally resizes image to fit within token budget
    ...
    Args:
        ...
        resize: If True, downsample to fit within token budget.
                If False, raise error if image doesn't fit.
    ...
    def _patchify(img: torch.Tensor) -> dict:
        ...
        # Step 1: Fit to token budget (resize if enabled)
        ...
        if (target_h, target_w) != (h, w):
            if not resize:
                ...
                w_p = math.ceil(w / patch)
                raise ValueError(
                    f"Image requires {h_p * w_p} patches but max_tokens={max_tokens}. "
                    f"Set resize=True to automatically downsample."
                )
        ...
        return {
            ...
            'time_idx': time_idx,
            'orig_height': torch.tensor(orig_height, dtype=torch.long),
            'orig_width': torch.tensor(orig_width, dtype=torch.long),
